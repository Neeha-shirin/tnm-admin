...
const PrivacyPolicy = () => {
  ...
  useEffect(() => {
    ...
    const updated = localStorage.getItem('privacyPolicyLastUpdated');
    if (saved) {
  ...
  const handleSave = async () => {
    if (!content.trim() || content === '<p>Enter your privacy policy content here...</p>') {
    ...
    try {
      ...
      localStorage.setItem('privacyPolicyLastUpdated', now);
      setSavedContent(content);
      setLastUpdated(now);
      setIsEditing(false);
      showNotification('Privacy Policy saved successfully!', 'success');
    } catch (error) {
      showNotification('Error saving privacy policy. Please try again.', 'error');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  // Delete
  const handleDelete = () => {
    if (window.confirm('Are you sure you want to delete the privacy policy? This action cannot be undone.')) {
      localStorage.removeItem('privacyPolicy');
      localStorage.removeItem('privacyPolicyLastUpdated');
      setContent('<p>Enter your privacy policy content here...</p>');
      setSavedContent('');
      setLastUpdated('');
      setIsEditing(false);
    ...
    <div className="">
      ...
      {notification.show && (
        <div className="fixed top-4 right-4 z-50 max-w-sm w-full">
          <div className={`relative bg-white shadow-2xl rounded-lg border-l-4 ${
            ...
            <div className="flex items-start">
              ...
              <div className="ml-3 flex-1">
                <p className={`text-sm font-medium ${
                  notification.type === 'error' ? 'text-red-800' : 'text-emerald-800'
                }`}>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white shadow-2xl rounded-2xl overflow-hidden mb-6 transform transition-all duration-300 hover:shadow-2xl border border-emerald-100">
          ...
          <div className="px-6 py-5 bg-gradient-to-r from-emerald-900 to-green-500 border-b border-emerald-400 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            <div>
              <h2 className="text-2xl font-bold text-white">Privacy Policy Editor</h2>
            ...
            {isEditing ? (
              <div className="space-y-4">
                <div className="flex justify-between items-center bg-emerald-50 px-4 py-3 rounded-xl border border-emerald-200 transform transition-all duration-300 hover:bg-emerald-100">
                  <span className="text-sm font-semibold text-emerald-800">Editing Mode</span>
                  <button 
              ...
              <div className="border-2 border-emerald-100 rounded-xl overflow-hidden transform transition-all duration-300 hover:border-emerald-200">
                <div className="bg-gradient-to-r from-emerald-50 to-green-50 px-4 py-4 border-b border-emerald-200 flex justify-between items-center">
                  <h3 className="text-xl font-bold text-emerald-900">Current Privacy Policy</h3>
                  {savedContent && (
                    <button 
                      onClick={handleEdit}
                ...
                <div className="p-6 min-h-[200px] bg-gradient-to-br from-white to-emerald-50">
                  {savedContent ? (
                    ...
                    <div className="text-center py-12 transform transition-all duration-500 hover:scale-105">
                      ...
                      <p className="text-emerald-600 text-xl font-semibold">No privacy policy created yet.</p>
                      <p className="text-emerald-500 mt-2">Click "Create Policy" to get started.</p>
                    </div>
                  )}
                </div>
              </div>
        ...
        {savedContent && (
          <div className="bg-white shadow-2xl rounded-2xl p-6 transform transition-all duration-300 hover:shadow-2xl border border-emerald-100">
            <h3 className="text-xl font-bold text-emerald-900 mb-6">Policy Information</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="text-center transform transition-all duration-300 hover:scale-105">
                <div className="text-sm font-semibold text-emerald-600 mb-2">Status</div>
              ...
              <div className="text-center transform transition-all duration-300 hover:scale-105">
                <div className="text-sm font-semibold text-emerald-600 mb-2">Last Updated</div>
                <div className="text-lg font-bold text-emerald-900 bg-emerald-50 px-4 py-3 rounded-xl border border-emerald-200">
                  {lastUpdated}
                </div>
              </div>
              <div className="text-center transform transition-all duration-300 hover:scale-105">
                <div className="text-sm font-semibold text-emerald-600 mb-2">Content Length</div>
                <div className="text-lg font-bold text-emerald-900 bg-emerald-50 px-4 py-3 rounded-xl border border-emerald-200">
        ...
        <div className="mt-6 bg-gradient-to-r from-emerald-50 to-green-50 rounded-2xl p-6 border border-emerald-200 transform transition-all duration-300 hover:shadow-lg">
          <div className="flex items-start">
            ...
            <div className="ml-4">
              <h3 className="text-lg font-bold text-emerald-800">Tips for your Privacy Policy</h3>
